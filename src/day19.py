...
class Solver(AbstractSolver):
    ...
    def read_input(self, lines: list) -> None:
        for line in lines:
            ...
            costs_matrix = [
                ...
                max(max(int(line_split[6]), int(line_split[12])), max(int(line_split[18]), int(line_split[27]))), # Max number of ore for anything.
    ...
    def key_for(self, num_rc):
        return "{} {} {} {} {} {} {} {}".format(num_rc[0], num_rc[1], num_rc[2], num_rc[3], num_rc[4], num_rc[5], num_rc[6], num_rc[7])
    ...
    def expand_paths_1(self, costs_matrix, pass_idx, current_dict, prev_dict):
        ...
        for rc in current_dict.values():
            ...
            for build_type in TYPES:
                ...
                if rc[ORE] >= costs_matrix_for_type[ORE] and rc[CLAY] >= costs_matrix_for_type[CLAY] and rc[OBSIDIAN] >= costs_matrix_for_type[OBSIDIAN]:
                    ...
                    new_rc = [
                        rc[ORE]
                        + rc[4]
                        - costs_matrix_for_type[ORE],
                        rc[CLAY]
                        + rc[5]
                        - costs_matrix_for_type[CLAY],
                        rc[OBSIDIAN]
                        + rc[6]
                        - costs_matrix_for_type[OBSIDIAN],
                        rc[GEODE]
                        + rc[7],
                        rc[4] + inc_4,
                        rc[5] + inc_5,
        ...
        for data in sorted_temp_dict:
            want_it = True
            item = data[1]
            if item[0] == prev_item[0] and item[1] == prev_item[1] and item[2] == prev_item[2] and item[3] == prev_item[3]:
                if item[4] <= prev_item[4] and item[5] <= prev_item[5] and item[6] <= prev_item[6] and item[7] <= prev_item[7]:
    ...
    def solve1(self):
        ...
        for id, costs_matrix in self.blueprints:
            ...
            best_scores.append((id, self.best_score_for_id, id * self.best_score_for_id))
            print("    ID {}, GEODES {}, QUALITY {}".format(id, self.best_score_for_id, id * self.best_score_for_id))

        final_score = 0
        print("BEST SCORES:")
        for score in best_scores:
            print("    ID {}, GEODES {}, QUALITY {}".format(score[0], score[1], score[2]))
            final_score += score[2]
        return final_score
